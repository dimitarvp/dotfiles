...
# Calculate line range to show (`bat` does not properly scroll to lines below initial view)
LINES=$(tput lines)
(( HALF_LINES = $LINES / 2 ))
(( LINE_START = $CENTER - $HALF_LINES ))
...
if [ -z "$FZF_PREVIEW_COMMAND" ] && [ "${BATNAME:+x}" ]; then
    ${BATNAME} --style="${BAT_STYLE:-numbers}" --color=always --pager=never \
               --highlight-line=$CENTER --line-range=$LINE_RANGE -- "$FILE"
    exit $?
fi
