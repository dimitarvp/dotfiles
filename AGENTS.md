...
- Files that chezmoi ignores on a machine (for example `adguard/` off the server role) are
  still public content; the ignore list controls deployment, not visibility.
- Fleet host names never appear in plaintext, file names included. A file that must name one
  is encrypted whole.
...
- `.chezmoiignore` lists what chezmoi must not deploy into `$HOME`. `AGENTS.md`, `CLAUDE.md`
  and `README.md` are in it; a new top-level file goes there too.
